@blog @delivery @release @cloudflare
Feature: DeployとProduct Releaseを独立して運用する
  運用者として
  ...
  Background:
    ...
    And Blogはflagを参照するだけで変更する権限を持たない
  ...
  Rule: versionはPRタイトルから決まる
    ...
    Scenario Outline: PRタイトルの型が必須bumpを決める
      ...
      Examples:
        | title                    | previous | next  |
        | feat: add a feed         | 0.1.0    | 0.2.0 |
        | perf: cache the feed     | 0.1.0    | 0.2.0 |
        | fix: correct the feed    | 0.1.0    | 0.1.1 |
        | revert: undo the feed    | 0.1.0    | 0.1.1 |
    ...
    Scenario: 要求されたbumpを行っていないPR
      ...
      And PRタイトルが "feat: add a feed" である
  ...
  Rule: 機能の公開はflags repositoryだけが行う
    ...
    Scenario: flagをONにして機能を公開する
      Given 対象flagを参照するBlogのversionがproductionへDeploy済みである
      When flags repositoryで対象flagをONにする
      Then Blogを再Deployせずに機能が公開される
      And Blogのversionとtagは変化しない

    Scenario: 公開済み機能を停止する
      Given 一つの機能がProduct Release済みである
      When flags repositoryで対象flagをOFFにする
      Then 読者はflag導入前の振る舞いを観測する
      And Blogのproduction Worker versionは変化しない
  ...
  Rule: 導入gateが閉じている間は外部状態を変更しない
    ...
    Scenario: Deploy gateが無効である
      ...
      When mainが更新される
      Then delivery workflowは意図的なskipを報告する
