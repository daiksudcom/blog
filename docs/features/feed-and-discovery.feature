...
Feature: 記事をフィードと検索エンジンへ公開する
  ...
  Scenario: RSS フィードを取得する
    When client が "https://blog.daiksud.me/rss.xml" を取得する
    Then 公開日時降順の記事 item を含む有効な RSS を受け取る
    And channel link と各 item link は "https://blog.daiksud.me" origin を使う
    And 各 item guid は恒久的な記事 URL である

  Scenario: sitemap から公開ページを発見する
    When crawler が "https://blog.daiksud.me/sitemap.xml" を取得する
    Then 記事ページと既知のタグページの canonical URL を発見できる

  Scenario: robots 方針を取得する
    When crawler が "https://blog.daiksud.me/robots.txt" を取得する
  ...
  Scenario: 記事の共有メタデータを得る
    ...
    Then canonical と Open Graph URL は同じ "https://blog.daiksud.me/{slug}/" である
