...
test('a version is only a plain SemVer core, never a tag or a prerelease', () => {
  ...
  assert.equal(formatSemver(parseSemver('0.9.0')), '0.9.0');
  for (const invalid of ['v0.1.0', '0.1', '0.1.0-rc.1', '0.1.0+20260813040210', '01.1.0']) {
    assert.throws(() => parseSemver(invalid), /plain SemVer core/, `${invalid} must be rejected`);
  }
  assert.ok(compareSemver('0.10.0', '0.9.0') > 0);
  assert.equal(compareSemver('0.1.0', '0.1.0'), 0);
  assert.ok(compareSemver({ major: 0, minor: 1, patch: 0 }, '1.0.0') < 0);
});

test('a subject is parsed into the parts the policy reasons about', () => {
  assert.deepEqual(parseCommitSubject('feat: add a feed'), {
    type: 'feat',
    scope: null,
    breaking: false,
    description: 'add a feed',
  });
  assert.deepEqual(parseCommitSubject('fix(feed)!: drop the legacy route'), {
    type: 'fix',
    scope: 'feed',
    breaking: true,
  ...
  assert.throws(() => parseCommitSubject('feat:missing space'), /not a Conventional Commit/);
});
