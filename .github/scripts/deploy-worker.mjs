...
/**
 * The feed is a separately rendered route that readers and crawlers depend on, so a
 * deploy that only proves the home page still renders would hide a broken `/rss.xml`.
 */
const SMOKE_PATHS = ['/', '/rss.xml'];

async function smoke(origin, paths, label) {
  const results = [];
  for (const path of paths) {
    const url = new URL(path, origin);
    url.searchParams.set('__blog_smoke', sha.slice(0, 12));
    const status = await withRetries(`${label} smoke ${url}`, async () => {
      try {
        const response = await fetch(url, {
          headers: { 'cache-control': 'no-cache', 'user-agent': 'blog-deploy-smoke/1' },
          redirect: 'follow',
