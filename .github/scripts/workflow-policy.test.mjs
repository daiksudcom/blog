...
test('the deploy smoke covers the feed as well as the home page', () => {
  const source = readFileSync(join('.github/scripts', 'deploy-worker.mjs'), 'utf8');
  assert.match(source, /__blog_smoke/, 'the smoke request must be identifiable in logs');
  assert.match(source, /blog-deploy-smoke\/1/);
  assert.match(source, /const SMOKE_PATHS = \['\/', '\/rss\.xml'\];/);
  assert.equal(
    source.match(/smoke\((?:uploaded\.preview_url|productionOrigin), SMOKE_PATHS,/g)?.length,
    2,
    'preview and production must smoke the same paths',
  );
});
