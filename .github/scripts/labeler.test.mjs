...
test('branch fixtures produce the intended synchronized labels', () => {
  const fixtures = [
    ['breaking-change/feat/article-routing', ['BREAKING CHANGE', 'feat']],
    ['deprecated/fix/legacy-feed', ['DEPRECATED', 'fix']],
    ['feat/tag-list', ['feat']],
    ['perf/isr-cache', ['perf']],
    ['fix/cursor-pagination', ['fix']],
    ['revert/bad-change', ['revert']],
    ['docs/adr-deploy', ['docs']],
    ['refactor/content-client', ['refactor']],
    ['style/format', ['style']],
    ['test/policy', ['test']],
...
// Deploy happens on every merged PR and Product Release is decided in the flags repository,
// never by a branch. A branch that merely looks like a release must stay inert.
...
test('path-only rules use one combined all-files glob', () => {
  ...
  const fixtures = [
    ...
    [['docs/features/blog-isr.feature'], ['docs']],
    [['README.md', 'src/pages/index.astro'], []],
    [['package.json', 'pnpm-lock.yaml'], ['build']],
    [['astro.config.mjs', 'wrangler.jsonc', 'tsconfig.json'], ['build']],
