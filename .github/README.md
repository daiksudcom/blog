...
Blogはfeature flagを保持しません。flagの状態はflags repositoryが正本として管理し、Blogはそれを参照して機能のオンオフを決めます。

したがってBlogで起こるproduction操作はDeployだけです。

- **Deploy**: PRがmergeされるたびに必ず実施する。`main`のrevisionをWorkerへ反映する。**機能は公開しない。** 不具合修正の反映は許容する。
- **Product Release**: flags repositoryでflagをONにすること。**Blogを再deployせずに機能が公開される。**
...
preview URLとproduction originの両方で`/`と`/rss.xml`を取得します。feedは記事一覧とは別に描画されるrouteであり、購読者とcrawlerが依存する契約なので、トップページだけのsmokeでは壊れたfeedを見逃します。
...
| `PRODUCTION_ORIGIN` | smoke対象の`https://blog.daiksud.me` |
...
**Deploy tokenにflag変更権限を与えてはいけません。** flagを変更できるのはflags repositoryだけです。GitHub workflowのdefault permissionはreadで、すべてのthird-party Actionはfull commit SHAで固定します。
